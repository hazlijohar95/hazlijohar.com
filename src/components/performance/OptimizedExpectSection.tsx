...
const Card = React.memo(({ icon, title, description }: CardProps) => (
  <motion.div
...
const OptimizedExpectSection = () => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isHovered, setIsHovered] = useState(false);
  const [isPaused, setIsPaused] = useState(false);

  // Memoize the current card to prevent unnecessary re-renders
  const currentCard = useMemo(() => expectCards[currentIndex], [currentIndex]);

  // Auto-advance logic with useCallback to prevent recreation
  const advanceToNext = useCallback(() => {
    if (!isHovered && !isPaused) {
      setCurrentIndex((prev) => (prev + 1) % expectCards.length);
    }
  }, [isHovered, isPaused]);

  // Enhanced auto-advance with pause capability
  useEffect(() => {
    const interval = setInterval(advanceToNext, 15000);
  ...
  const handleDotClick = useCallback((index: number) => {
    ...
    setIsPaused(true);
    // Resume auto-advance after 30 seconds
              ...
              >
                ...
                <div className="absolute -bottom-16 left-0 right-0 h-px bg-black/20">
                  <motion.div
                    ...
                    initial={{ width: "0%" }}
                    animate={{
                      width: (isHovered || isPaused) ? "0%" : "100%"
                    }}
                    transition={{
                      duration: (isHovered || isPaused) ? 0.3 : 15,
                      ease: "linear",
                      repeat: (isHovered || isPaused) ? 0 : Infinity,
                      repeatType: "loop"
                    }}
                  />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl">
              {[
                ...
                >
                  ...
                  <div className="font-mono text-xs text-[#666] uppercase tracking-wide">
                    {stat.label}
                  </div>
                </motion.div>
              ))}
...
export default OptimizedExpectSection;